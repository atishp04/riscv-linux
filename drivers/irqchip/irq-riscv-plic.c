...
static int plic_nr_handlers;
static bool plic_handler_present[MAX_CONTEXTS];
...
static inline void plic_irq_toggle(struct irq_data *d, int enable)
{
	...
	for (i = 0; i < plic_nr_handlers; i++) {
		if (plic_handler_present[i])
			plic_toggle(i, d->hwirq, enable);
	}
}
		...
		struct device_node *parent)
{
	...
	plic_nr_handlers = of_irq_count(node);
	if (WARN_ON(!plic_nr_handlers))
		goto out_iounmap;
	if (WARN_ON(plic_nr_handlers < num_possible_cpus()))
		goto out_iounmap;
	...
	for (i = 0; i < plic_nr_handlers; i++) {
		...
		if (!of_device_is_compatible(parent.np, "riscv,cpu-intc") ||
		    !parent.np->parent) {
			pr_err("invalid OF parent, skipping context %d.\n", i);
			continue;
		}
	...
	pr_info("mapped %d interrupts to %d (out of %d) handlers.\n",
		nr_irqs, nr_mapped, plic_nr_handlers);
